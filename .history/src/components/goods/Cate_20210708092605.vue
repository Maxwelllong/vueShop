...
export default {
    ...
    methods:{
        ...
        handleChange(){
            ...
            const res = this.selectedOption
            if(res) return this.$message.warning('请选择对应的父节点');
            this.cat_pid = res[res.length -1]
            this.cat_level = res.length
        },
        updataCateDialog(){
            this.$refs.addCateFormRef.validite(async(valid)=>{
                if(valid){
                    const { data: res } = await this.$http.post(`categories`, this.addCateForm)
                    if(res.meta.status !== 200 ) return this.$message.warning('添加商品分类失败')
