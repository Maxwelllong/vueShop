    ...
    <el-card>
        ...
        show-index index-text='序号' :expand-type='false' border>
            ...
            <template slot-scope="scope" slot='toOrder'>
                <el-tag type='success' v-if='scope.row.cat_pid === 0 '>一级</el-tag>
                <el-tag type='warning' v-else-if="scope.row.cat_pid === 1">二级</el-tag>
                <el-tag type='danger' v-else-if="scope.row.cat_pid === 2">三级</el-tag>
            </template>
        </tree-table>
    </el-card>
    <!-- 底部分页功能 -->
