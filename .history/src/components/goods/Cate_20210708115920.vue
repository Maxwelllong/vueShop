...
<template>
    ...
    <el-dialog title="修改分类名称" :visible.sync="editDialog"  width="50%">
        <el-form ref='inputTable' :model="queryeditForm" :rules="editCateFormRules" >
            <el-form-item prop="cat_name" >
                 <el-input v-model="queryeditForm.cat_name" clearable ></el-input>
            </el-form-item>
        </el-form>
        <span slot="footer" class="dialog-footer">
            <el-button @click="editDialog = false">取 消</el-button>
            <el-button type="primary" @click="updataEditDialog">确 定</el-button>
        </span>
    </el-dialog>
...
export default {
    ...
    methods:{
        ...
        async editTableItem(item){
            ...
            this.queryeditForm.id = res.data.cat_id;
            this.queryeditForm.cat_name = res.data.cat_name;
            this.editDialog = true;
        },
        // 提交编辑按钮
        updataEditDialog(){
            this.$refs.inputTable.validate(async(valid)=>{
                if(valid){
                    const {data:res} = await this.$http.put(`categories/${this.queryeditForm.id}`,this.queryeditForm)
                    if(res.meta.status !== 200) return this.$message.warning('请求数据失败')
                    this.$message.success('修改成功')
                    this.getTablelist();
                    this.editDialog = false;
                }else{
                    return this.$message.warning('请重新检查是否填写正确')
                }
            })
        },
        // 删除按钮
        async deleteBtn(){ 
        const confirm = await this.$confirm('此操作将永久删除此分类, 是否继续?', '提示', {
        ...
        if(confirm === 'confir'){
            const {data :res} = await this.$http.delete(`categories/${this.queryeditForm.cat_name}`)
            if(res.meta.status !== 200){
                this.$message.warning('删除失败，请刷新重试')
            }else{
