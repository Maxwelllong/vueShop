...
<template>
    ...
    <el-dialog title="修改分类名称" :visible.sync="editDialog"  width="50%">
        <el-form ref='inputTable' :rules="addCateFormRules" >
        <el-input v-model="queryeditForm.editCateName" prop="addCateName" clearable ></el-input>
        </el-form>
        <span slot="footer" class="dialog-footer">
...
export default {
    ...
    methods:{
        ...
        async editTableItem(item){
            ...
            this.queryeditForm.editCateName = res.data.cat_name;
        ...
        updataEditDialog(){
            this.$refs.inputTable.validate(async(valid)=>{
                if(valid){
                    const {data:res} = await this.$http.put(`categories/${this.queryeditForm.id}`)
