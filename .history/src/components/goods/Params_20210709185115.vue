                ...
                <el-table :data="manyParamsList" >
                    ...
                    <el-table-column label="操作" > 
                        <template slot-scope="scope" class='deleAndeditBtn'>
                            ...
                            <el-button type="danger"  size='mini' icon="el-icon-delete" @click="deleEditInfo(scope.row)">删除</el-button>
                        </template>
                    </el-table-column>
                </el-table>
            </el-tab-pane>
                ...
                <el-table :data="onlyParamsList" >
                    <el-table-column type='expand' width="80" > 
                       <template slot-scope="scope">
                            <el-tag :key="tag" v-for="tag in addTags" closable :disable-transitions="false" @close="handleClose(tag)">
                                {{tag}}
                            </el-tag>
                       </template>
                    </el-table-column>
    ...
    methods:{
        ...
        async getParamsList(id,sel){  
            ...
            if(this.activeName === 'many'){
                res.data = (res.data).forEach(item =>{
                    item.attr_vals = item.attr_vals.splict(',')
                })
        ...
        async updataAddCate(){
            this.$refs.editCateFormRef.validate( async valid =>{
                ...
                { attr_name : this.editCateForm.attr_name , attr_sel : this.activeName}
                )
                if(res.meta.status !== 200 ) return this.$message.warning('修改信息失败')
                this.$message.success('修改信息成功')
                this.handleChange()
                this.editDialog = false ;
            })
        },
        async deleEditInfo(item) {
