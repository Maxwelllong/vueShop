                ...
                <el-table-column type='expand'>
                    <template slot-scope="scope">
                        <el-row v-for="(item,index) in scope.row.children" :key="item.id" class='bdStyle toCenter'  >
                            <el-col :span="5" >
                                <el-tag type='success' closable @close="closeTagbar(scope.row.id,item.id)">{{item.authName}}</el-tag>  
                            ...
                            <el-col :span="19" >
                                <el-row v-for="(tags,indexs) in item.children" :key="tags.id" :class="indexs==0 ? '':'bdStyle'" class='toCenter'>
                                    <el-col :span="6">
                                        <el-tag type='primary' closable @close="closeTagbar(scope.row.id,tags.id)" >{{tags.authName}}</el-tag>
                                        <el-icon class='el-icon-caret-right'></el-icon>
                                    </el-col>
                                    <el-col :span="11">
                                        <el-tag closable style='margin:10px' effect="dark" v-for="(tag,idx) in tags.children" @close="closeTagbar(scope.row.id,tag.id)"
...
export default {
    ...
    methods:{
        ...
        async closeTagbar(roleId,rightId){
            // console.log(roleId);
            // console.log(rightId);
