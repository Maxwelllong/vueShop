    ...
    <div>
       <!-- 面包屑导航 -->
        ...
        <el-card>
            ...
            <el-table :data="userInfo" stripe style="width: 100%" border>
                ...
                <el-table-column type='expand'>
                    <template slot-scope="scope">
                        <el-row v-for="(item) in scope.row.children" :key="item.id" class='bdStyle toCenter'  >
                            <el-col :span="5" >
            ...
            <!-- 添加权限 -->
            <el-dialog title="修改权限"  :visible.sync="setRightDialog" width="50%">
               <el-tree :data="setRightForm " default-expand-all show-checkbox node-key="id" ref="treeRef"
...
export default {
    ...
    methods:{
        ...
        updateEditForm(){
            ...
            // { data: res } = await this.$http.post(`roles/:roleId/rights`, {
            //     rids : checkedArr.join(',')
            // })
            // if(res.meta.status !== 200)return this.$message.warning('修改权限失败')
