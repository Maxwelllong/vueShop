            ...
            <el-table :data="userInfo" stripe style="width: 100%" border>
                <el-table-column type='expand'>
                    <template slot-scope="scope">
                        <el-row v-for="(item,index) in scope.row.children" :key="item.id" class='bdStyle' :class='btm'>
                            <el-col :span="5" >
                                <el-tag type='success' closable>{{item.authName}}</el-tag>
                                <el-icon class='el-icon-caret-right'></el-icon>
                            </el-col>
                            <el-col :span="19">
                                <el-row :span="8" v-for="(tags,indexs) in item.children" :key="tags.id" :class="indexs==0 ? '':'bdStyle'">
                                    <el-col :span="6">
