...
<template>
    <div>
                    ...
                    <el-table-column label="操作" > 
                        <template slot-scope="scope" >
                            ...
                            <el-tooltip  content="分配角色" placement="top" :enterable='false'>
                                <el-button size='mini'  type="primary" icon='el-icon-setting' @click="setRoles(scope.row)"></el-button>
                            </el-tooltip>
                        </template>
                    </el-table-column>
                 </el-table> 
        ...
        <!-- 设置用户角色 -->
        <el-dialog title="分配用户角色" :visible.sync="setRoleDialog" width="50%">
        <div> 当前用户名：{{userInfo.username}}</div>
        <div> 当前用户名：{{userInfo.role_name}}</div>
        <div> 分配新角色：
            <el-select v-model="selectdRole" placeholder="请选择">
                <el-option v-for="item in rolesOption" :key="item.id"
                 :label="item.username" :value="item.role_name">
                </el-option>
            </el-select> 
        </div>
        <span slot="footer"
        ...
        <el-button type="primary"
                    @click="setRoleDialog= false">确 定</el-button>
        </span>
        </el-dialog>
...
export default {
    ...
    methods:{
        ...
        async deleteUser(id){
            ...
            if(res.meta.status !==200){
                return this.$message.error('用户删除失败或不能删除')
            }
        ...
        async setRoles(Info){
            ...
            this.rolesOption = res.data.users
            // this.rolesOption = 
        }
    }
}
