...
<template>
    <div>
        ...
        <div><p> 当前用户名：{{userInfo.username}}</p></div>
        <div><p> 当前角色：{{userInfo.role_name}}</p></div>
        <div> 分配新角色：
            <template>
              <el-select v-model="selectedRoles" placeholder="请选择">
                <el-option v-for="item in rolesOption" :label="item.roleName" :value="item.id"
                :key="item.id">
                </el-option>
              </el-select>
            </template>
...
export default {
    ...
    methods:{
        ...
        saveRoleInfo(){
            ...
            const res = this.$http.put(`user/${this.userInfo.id}/role`, {
                rid:this.selectedRoles
            });
            if(res.meta.status !== 200)  return this.$message.warning('修改角色失败')
            console.log(res);
            setRoleDialog= false;
        }
